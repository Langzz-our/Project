...
const PRODUCTS = [
  { id:"1", name:"Obsidian Chronograph", price:"4250000", comparePrice:"5500000", image:"https://images.unsplash.com/photo-1523275335684-37898b6baf30?w=600&q=80", featured:true, category:"Fashion" },
  { id:"2", name:"Noir Leather Tote", price:"1890000", comparePrice:null, image:"https://images.unsplash.com/photo-1548036328-c9fa89d128fa?w=600&q=80", featured:true, category:"Fashion" },
  { id:"3", name:"Phantom ANC Pro", price:"3200000", comparePrice:"3800000", image:"https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=600&q=80", featured:true, category:"Elektronik" },
  { id:"4", name:"Studio Desk Lamp", price:"890000", comparePrice:null, image:"https://images.unsplash.com/photo-1507473885765-e6ed057f782c?w=600&q=80", featured:false, category:"Home" },
  { id:"5", name:"Glacier Skincare Set", price:"1450000", comparePrice:"1750000", image:"https://images.unsplash.com/photo-1556228578-8c89e6adf883?w=600&q=80", featured:true, category:"Beauty" },
  { id:"6", name:"Merino Cashmere Coat", price:"6750000", comparePrice:"8200000", image:"https://images.unsplash.com/photo-1539533018447-63fcce2678e3?w=600&q=80", featured:true, category:"Fashion" },
];
...
function useInView(t=0.15){const ref=useRef(null);const[v,setV]=useState(false);useEffect(()=>{const o=new IntersectionObserver(([e])=>{if(e.isIntersecting){setV(true);o.disconnect();}},{threshold:t});if(ref.current)o.observe(ref.current);return()=>o.disconnect();},[]);return{ref,visible:v};}

function Navbar(){
  const[scrolled,setScrolled]=useState(false);const[open,setOpen]=useState(false);const{total}=useCart();
  ...
  return(<>
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:100,padding:scrolled?"10px 20px":"20px 20px",background:scrolled?"rgba(5,5,5,0.95)":"transparent",backdropFilter:scrolled?"blur(24px)":"none",borderBottom:scrolled?"1px solid rgba(255,255,255,0.06)":"none",display:"flex",alignItems:"center",justifyContent:"space-between",transition:"all 0.5s"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:14}}>
        <button style={{background:"none",border:"none",cursor:"pointer",color:"rgba(255,255,255,0.5)",display:"flex"}}><Search size={18}/></button>
        <button style={{background:"none",border:"none",cursor:"pointer",color:"rgba(255,255,255,0.5)",display:"flex",position:"relative"}}>
          <ShoppingBag size={18}/>
          {total()>0&&<span style={{position:"absolute",top:-6,right:-6,width:16,height:16,background:"#fbbf24",color:"#000",fontSize:9,fontWeight:800,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center"}}>{total()}</span>}
        </button>
        <button onClick={()=>setOpen(!open)} style={{background:"none",border:"none",cursor:"pointer",color:"rgba(255,255,255,0.6)",display:"flex"}}>{open?<X size={20}/>:<Menu size={20}/>}</button>
      </div>
    </nav>
  ...
  return(
    <section style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"#060606",position:"relative",overflow:"hidden",padding:"100px 24px 60px"}}>
      ...
      <div style={{position:"relative",textAlign:"center",maxWidth:520,opacity:v?1:0,transform:v?"translateY(0)":"translateY(30px)",transition:"all 1s cubic-bezier(0.4,0,0.2,1)"}}>
        ...
        <p style={{color:"rgba(255,255,255,0.4)",fontSize:15,lineHeight:1.75,marginBottom:36}}>Koleksi produk premium — fashion, elektronik, beauty. Kualitas tanpa kompromi.</p>
        <div style={{display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap",marginBottom:48}}>
          <a href="#products" style={{display:"inline-flex",alignItems:"center",gap:10,padding:"14px 26px",background:"linear-gradient(135deg,#fbbf24,#f59e0b)",color:"#000",fontWeight:700,fontSize:14,borderRadius:99,textDecoration:"none",boxShadow:"0 0 30px rgba(251,191,36,0.25)"}}>Jelajahi Koleksi <ArrowRight size={15}/></a>
          <a href="#" style={{display:"inline-flex",alignItems:"center",padding:"14px 26px",border:"1px solid rgba(255,255,255,0.12)",color:"rgba(255,255,255,0.8)",fontWeight:600,fontSize:14,borderRadius:99,textDecoration:"none"}}>Lihat Lookbook</a>
        </div>
...
function ValueProps(){
  ...
  const items=[{Icon:Truck,title:"Free Shipping",desc:"Gratis ongkir di atas Rp500.000"},{Icon:Shield,title:"Garansi Resmi",desc:"Semua produk terverifikasi asli"},{Icon:Zap,title:"Fast Processing",desc:"Dikirim dalam 24 jam kerja"}];
  return(
...
function ProductCard({product,delay=0}){
  const[liked,setLiked]=useState(false);const[added,setAdded]=useState(false);const[pressed,setPressed]=useState(false);
  const{add}=useCart();const{ref,visible}=useInView(0.1);
  return(
    <div ref={ref} style={{opacity:visible?1:0,transform:visible?"translateY(0)":"translateY(24px)",transition:`all 0.6s ease ${delay}s`}}>
      ...
      >
        <div style={{position:"relative",aspectRatio:"1/1",overflow:"hidden",background:"#1a1a1a"}}>
          ...
          <button onClick={()=>setLiked(!liked)} style={{position:"absolute",top:12,right:12,width:34,height:34,borderRadius:"50%",background:"rgba(0,0,0,0.5)",backdropFilter:"blur(8px)",border:"1px solid rgba(255,255,255,0.1)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>
        ...
        <div style={{padding:"14px"}}>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:3,background:"rgba(251,191,36,0.08)",padding:"3px 8px",borderRadius:99}}>
              <Star size={9} fill="#fbbf24" color="#fbbf24"/><span style={{color:"#fbbf24",fontSize:10,fontWeight:600}}>4.8</span>
            </div>
          </div>
          <button onClick={()=>{add({id:product.id,name:product.name,price:product.price,image:product.image});setAdded(true);setTimeout(()=>setAdded(false),2000);}}
  ...
  return(
    <section ref={ref} style={{padding:"0 20px 72px",background:"#060606"}}>
      <div style={{maxWidth:900,margin:"0 auto",background:"linear-gradient(135deg,rgba(251,191,36,0.08),rgba(251,191,36,0.03))",border:"1px solid rgba(251,191,36,0.15)",borderRadius:24,padding:"40px 24px",textAlign:"center",opacity:visible?1:0,transform:visible?"translateY(0)":"translateY(24px)",transition:"all 0.7s ease",position:"relative",overflow:"hidden"}}>
        ...
        <p style={{color:"rgba(255,255,255,0.4)",fontSize:13,lineHeight:1.7,marginBottom:24}}>Bergabung dengan 50.000+ member dan nikmati akses eksklusif ke koleksi terbaru.</p>
